...
{
	DFBCHECK(dfb_interface->CreateFont(dfb_interface, "/home/galois/fonts/Vera.ttf", &font_desc, &font_interface));
...
static GraphicsControllerError drawInfo()
{
	...
	/*  draw the frame  for channel number*/
	DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xFF));
	DFBCHECK(primary->FillRectangle(primary, 591, 960 , 336, 87.75));

	/*drawing a box for channel number */
	DFBCHECK(primary->SetColor(primary, 0x99, 0x00, 0x99, 0xFF));
	DFBCHECK(primary->FillRectangle(primary, 591 + FRAME_THICKNESS, 960 + FRAME_THICKNESS, 336 - 2 * FRAME_THICKNESS, 87.75 - 2 * FRAME_THICKNESS));
	//DFBCHECK(primary->FillRectangle(primary, screen_width / 6 + FRAME_THICKNESS, screen_height / 1.125  + FRAME_THICKNESS, 14 * screen_width / 80 - 2 * FRAME_THICKNESS, 1.3 * screen_height / 16 - 2 * FRAME_THICKNESS));

	/* Channel */
	sprintf(text_string, "Channel: %d", program_number);
	setFontSize(FONT_HEIGHT/2);
	DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xff));
	DFBCHECK(primary->DrawString(primary, text_string, -1, 271 + 350, 960 + 3 * FONT_HEIGHT / 2, DSTF_LEFT));

	/*  draw the frame  for video pid*/
	DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xFF));
	DFBCHECK(primary->FillRectangle(primary, 957, 960, 168, 87.75));

	/*drawing a box for video pid */
	DFBCHECK(primary->SetColor(primary, 0x99, 0x00, 0x99, 0xFF));
	DFBCHECK(primary->FillRectangle(primary, 957 + FRAME_THICKNESS, 960 + FRAME_THICKNESS, 168 - 2 * FRAME_THICKNESS, 87.75 - 2 * FRAME_THICKNESS));
	
	/* Video pid */
	setFontSize(FONT_HEIGHT);
	DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xFF));
	sprintf(text_string, "Video PID:");
	DFBCHECK(primary->DrawString(primary, text_string, -1, 271 + 722, 954 + FONT_HEIGHT, DSTF_LEFT));
	if (!radio)
	{
		sprintf(text_string, "%d", video_pid);
	}
	else 
	{
		sprintf(text_string, "NaN");
	}
	setFontSize(FONT_HEIGHT);
	DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xFF));
	DFBCHECK(primary->DrawString(primary, text_string, -1, 271 + 728, 990 + FONT_HEIGHT, DSTF_LEFT));

	/*  draw the frame  for audio pid*/
	DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xFF));
	DFBCHECK(primary->FillRectangle(primary, 1155, 960 , 168, 87.75));

	/*drawing a box for video pid */
	DFBCHECK(primary->SetColor(primary, 0x99, 0x00, 0x99, 0xFF));
	DFBCHECK(primary->FillRectangle(primary, 1155 + FRAME_THICKNESS, 960 + FRAME_THICKNESS, 168 - 2 * FRAME_THICKNESS, 87.75 - 2 * FRAME_THICKNESS));

	/* Audio pid */
	sprintf(text_string, "Audio PID:");
	setFontSize(FONT_HEIGHT/2);
	setFontSize(FONT_HEIGHT);
	DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xFF));
	DFBCHECK(primary->DrawString(primary, text_string, -1, 271 + 914, 954 + FONT_HEIGHT, DSTF_LEFT));

	setFontSize(FONT_HEIGHT);
	sprintf(text_string, "%d", audio_pid);
	DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xFF));
	DFBCHECK(primary->DrawString(primary, text_string, -1, 271 + 926, 990 + FONT_HEIGHT, DSTF_LEFT));

	return GC_NO_ERROR;
}

static GraphicsControllerError drawAudioVideoText(char* text)
{
	int32_t ret;
	char img_name[50];
...
static GraphicsControllerError drawSoundVolume()
{
	...
	DFBCHECK(primary->Blit(primary,
						   /*source surface*/ logo_surface,
						   /*source region, NULL to blit the whole surface*/ NULL,
						   /*destination x coordinate of the upper left corner of the image*/ 772,
						   /*destination y coordinate of the upper left corner of the image*/ 50));
